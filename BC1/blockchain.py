...
class BlockChain:
    ...
    def valid_transactions(self, transactions: List[Dict[str, Any]]) -> bool:
    	#todo
        return True
    ...
    def resolve_conficts_transaction(self) -> bool:
        ...
        for node in self.nodes:
            ...
            if response.status_code == 200:
                j_data = response.json()
                length = j_data['length']
                transactions = j_data['transaction']
                if (length > max_length) and self.valid_transactions(transactions):
                    max_length = length
